        ...
        if (response.length < 1) {
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("custom")) {
                    ...
                    //check if the custom payload type is "datepicker"
                    if (response[i].custom.payload == "datePicker") {
                        renderDatePicker();
                        return;
                    }

                    //check if the custom payload type is "datepicker 2"
                    if (response[i].custom.payload == "datePick") {
                        renderDatePick();
                        return;
                    }
...
function renderDropDwon(data) {
    ...
    var select = '<div class="dropDownMsg"><select class="browser-default dropDownSelect"> <option value="" disabled selected>Choose eshaan</option>' + options + '</select></div>'
    $(".chats").append(select);
    scrollToBottomOfResults();
...
function createCardsCarousel(cardsData) {
    ...
    for (i = 0; i < cardsData.length; i++) {
        ...
        desc = cardsData[i].descriptions;
        ks = cardsData[i].toh
        ratings = Math.round((cardsData[i].ratings / 5) * 100) + "%";
        data = cardsData[i];
        itemsd = function getInputValue(){
                 var datese = "asType"
                 setUserResponse(datese);
                 send(datese);
                 $(".paginated_cards").remove();
                 scrollToBottomOfResults();

                 }
        item = '<div class="carousel_cards in-left">' + '<a href="' + ks + '"  target="_blank"><img class="cardBackgroundImage" src="' + cardsData[i].image + '"> + "</a> " + <div class="cardFooter">' + '<a href="' + ks + '"  target="_blank"><span class="cardTitle" title="' + title + '">' + title + "</span></a>" + '<span class="cardTitleesh">' + desc + "</span>" + '<button id="butt" name="submit" type="submit" onclick="getInputValue();">View Amenities</button>' + '<script>' + itemsd + '</script>' + "</div>" + "</div>";
...
function renderDatePick() {
    inputs = function getInputValue(){
             var dates = document.getElementById("datepick").value;
             var output = dates.replace(/(\d{4})-(\d\d)-(\d\d)/, "$3-$2");
    ...
    buttons = '<button id="butt" name="submit" type="submit" onclick="getInputValue();">submit</button>'
    var selects = '<div class="dropDownMsg">' +
                 '<input type="date" id="datepick" value="" />' + buttons +
                 '<script>' +
                  inputs
                 '</script>' +
                 '</div>'
    $(".chats").append(selects);
    scrollToBottomOfResults();
}
